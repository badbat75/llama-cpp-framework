...
| `server.ini` | `%LOCALAPPDATA%\llama.cpp\config\` | Machine-wide llama-server runtime (port, host, threads, threads-batch, cache-reuse, mlock, models dir, max simultaneous models, GPU device, multi-GPU split via split-mode / tensor-split). Single `[Server]` section. Written by `llama-cpp-config`. |
