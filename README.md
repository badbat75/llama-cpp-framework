...
| `server.ini` | `%LOCALAPPDATA%\llama.cpp\config\` | Machine-wide llama-server runtime (port, host, threads, mlock, models dir, max simultaneous models). Single `[Server]` section. Written by `llama-cpp-config`. |
| `presets.ini` | `%LOCALAPPDATA%\llama.cpp\config\` | Per-model presets: one `[<model-id>]` section each with model path, ctx size, GPU layers, batch sizes, cache types, flash-attn, sampling, MoE expert offload, chat template. Consumed directly by `llama-server --models-preset`. Written by `llama-cpp-config`. |
