...
Modal dialogs (New/Clone picker, Rename, the Delete confirmation, and the discard-confirm guard) overlay the whole window: Esc or a backdrop click dismisses them, and the app-wide shortcuts (Ctrl+S, Ctrl+N, Ctrl+1–3, F5) are inert while one is open (`AppState.modal_open`). Any navigation that would replace a dirty form — switching presets, Refresh, opening New…/Clone… — first asks to discard the unsaved edits (`gui::confirm_discard_then` parks the action Rust-side until the verdict).
