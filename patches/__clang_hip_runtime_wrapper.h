...
/* Use a different guard so the build system can suppress the original via
   -D__CLANG_HIP_RUNTIME_WRAPPER_H__ while still loading this patched copy. */
#ifndef __CLANG_HIP_RUNTIME_WRAPPER_PATCHED_H__
#define __CLANG_HIP_RUNTIME_WRAPPER_PATCHED_H__

/* Also mark the original guard as seen to prevent the stock wrapper from
   being included later (the HIP driver -include's it unconditionally). */
...
/* ── PATCH: HIP device math headers BEFORE standard library headers ──
   The forward-declares file comment states it MUST be included before <cmath>
   to prevent MSVC's constexpr math functions from becoming __host__ __device__
   and conflicting with __device__ implementations.
   We include <math.h> (C header, no __clang__ conflicts) early to provide
   FP_NAN / FP_INFINITE etc. that __clang_hip_cmath.h requires. */
...
/* ── Standard library headers AFTER HIP device math headers ── */
#if !defined(__HIPCC_RTC__)
#include <cmath>
#include <cstdlib>
#include <stdlib.h>
...
#endif // __HIPCC_RTC__
#endif // __HIP__
#endif // __CLANG_HIP_RUNTIME_WRAPPER_PATCHED_H__
